...
public class CreateTrendsReportServerTaskPreferencesDialogControl extends TweetExtractorFXDialogController {
	...
	public void setMainApplication(MainApplication mainApplication) {
		...
		toggleGroup.selectedToggleProperty().addListener((observable, oldValue, newValue) ->{
			if(newValue.equals(wordsChoice)) {
				nSlider.setMax(750);
				nSlider.setValue(350);
				nSlider.setMajorTickUnit(100);
				nSlider.setSnapToTicks(false);
				filterList.clear();
				filterToAdd.setDisable(true);
			}else {
				nSlider.setMax(10);
				nSlider.setValue(5);
				nSlider.setMajorTickUnit(1);
				nSlider.setSnapToTicks(true);
				filterToAdd.setDisable(false);
			}
		});
	}
	...
	public void onDone() {
		...
		if(hashtagChoice.equals(toggleGroup.getSelectedToggle())) {
			...
			CreateTrendsReportServerTaskPreferencesDialogResponse wordsReportResponse= (CreateTrendsReportServerTaskPreferencesDialogResponse) this.getMainApplication().showDialogLoadFXML("view/server/dialog/CreateTrendingWordsReportServerTaskPreferencesDialog.fxml", CreateTrendingWordsReportServerTaskPreferencesDialogControl.class);
			((CreateTrendsReportServerTaskPreferencesDialogResponse) this.getResponse()).setLanguageID(wordsReportResponse.getLanguageID());
			((CreateTrendsReportServerTaskPreferencesDialogResponse) this.getResponse()).setCustomStopWordsListName(wordsReportResponse.getCustomStopWordsListName());
		}
